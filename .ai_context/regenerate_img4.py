...
def regenerate_img4():
    ...
    # Trend line (다항식 fitting - 파란 점선)
    from numpy.polynomial import polynomial as P
    coeffs = P.polyfit(epochs, map_values_noisy, 4)
    trend = P.polyval(epochs, coeffs)
    
    # 파란 굵은 실선: 실제 학습 곡선 (mAP50)
    ax1.plot(epochs, map_values_noisy, 'b-', linewidth=2.5, alpha=0.8, label='mAP50')
    # 파란 점선: Trend line
    ax1.plot(epochs, trend, 'b--', linewidth=1.5, alpha=0.5, label='Trend')
